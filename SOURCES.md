...
| 2026-09-18 | T. F. Adesiyan (maintainer) | **Reverses the design decision recorded on 16 September.** **The API state sweep is substantially incomplete; the bulk download is the archive's capture from now on.** Both routes were exercised on the same day and compared directly. Bulk download: agritourism 13,569, farmers market 7,148, on-farm market 4,692, CSA 2,002, food hub 480 — matching USDA's own published totals exactly, with no duplicate `listing_id`. API state sweep, same day: 10,389 / 5,687 / 1,401 / 753 / 185, or 77%, 80%, **30%**, **38%**, **39%**. **Not a result cap:** no per-query limit from 50 to 1,000, applied to the actual per-state distribution parsed from `location_address`, reproduces the pattern, and for CSA and food hub the API returned fewer rows than a limit of 50 would yield. There is no state column in the data at all — `location_address` is free text, inconsistently formatted (`Wenatchee, WA` beside `509 Scott St, Little Rock, Arkansas 72201`, and a recurring `Massachussetts` misspelling) — so the `state` parameter must filter on a derived field populated for only part of the register. Which records it is blind to cannot be determined from outside. **Two further findings:** the bulk download carries 85–264 columns against the API's nine, and every product, facility, season, production-method and sales-channel field exists only in the download; and the bulk download contains **no contact email or telephone field at all** (`saleschannel_phoneorder` is a yes/no flag), so the redaction policy adopted earlier the same day is moot for the route now preferred. Coordinates are present for 99.3% of listings, leaving an `x`/`y`/`radius` grid sweep as a possible future automation — recorded in ROADMAP, not attempted. Recorded as CHARTER.md standard 9: completeness outranks automation. |

| 2026-09-18 | T. F. Adesiyan (maintainer) | **v0.1.0 released and deposited.** Verification checklist completed and signed; publish gate run; DRAFT stamps removed. Zenodo deposit minted concept DOI `10.5281/zenodo.22834813` (resolves to the newest version) and version DOI `10.5281/zenodo.22834814` (v0.1.0 permanently). **Two defects were found in the publish gate itself during this step and are recorded because a gate that fails silently is worse than no gate.** First, its placeholder scan listed placeholders by name, so it passed a repository containing `[project URL]` and `[repository URL]`; it now matches by shape. Second, and seriously, its DRAFT-stripping pattern used `.` under `re.DOTALL` and so matched from the banner to the end of the file — it deleted 222 of `CHARTER.md`'s 231 lines and reported success. The charter was restored from a pre-run copy. The pattern is now line-anchored, and the script refuses to write any file where a DRAFT pattern wants to remove more than twelve lines, tested by reintroducing the original bug. |
